"""
This module contains callback functions that handle the second linear modelization of the analysis.
...
@callback(
    Output("graph-second_linear", "figure", allow_duplicate=True),
    Output("title_analysis_second_linear", "children"),
    Output('slider-div-outlier-second_linear', "children"),
    ...
    Input("div-second_linear", "children"),
    State("test", "data"),
    State("back", "data"),
    State("initial_df_third_modelization", "data"),
    prevent_initial_call='initial_duplicate'
)
def display_figure_second_linear(graph, test_id, bool_back, initial_df):
    """
    Displays the second linear analysis figure along with the associated outlier slider and data.
    ...
    initial_df : list
        The initial dataframe used for third modelization if returning to a previous step.
    ...
    try:
        ...
        slider_outlier = dcc.Slider(
            id="slider-outlier-second_linear",
            min=0, 
            max=200, 
            step=1, 
        ...
        logging.error(f"Error in display_figure_second_linear : {e}/n{traceback.format_exc()}")
        return (
...
@callback(
    ...
    Output('data_outliers', "data", allow_duplicate=True),
    Output('data_vo2', 'data', allow_duplicate=True),
    Output("slider-div-protect-left-second_linear", "children"),
    Output("slider-div-protect-right-second_linear", "children"),
    Output("error-toast-app", "is_open", allow_duplicate=True),
    Output("error-toast-app", "children", allow_duplicate=True),
    Input('graph-second_linear', 'clickData'),
    Input("dropdown-model-second_linear", "value"),
    State('graph-second_linear', 'figure'),
    State('test', 'data'),
    State('data_vo2', 'data'),
...
def update_figure_second_linear(click_data, modelization_type, figure, test_id, data_vo2, n_outliers, initial_df, bool_back):
    """
    Updates the second linear analysis figure based on user interactions such as clicking on the graph 
    ...
    modelization_type : str
        The selected modelization type, either "first-model-second_linear" or "second-model-second_linear".
    figure : dict
    ...
    initial_df : list
        The initial dataframe for third modelization if returning to a previous analysis step.
    bool_back : bool
    ...
    try:
        ...
        if figure:
            ...
            slider_left = create_slider(
                ...
                "slider-bar-protect-left-second_linear", 
                3, 
                time=True
            )
            slider_right = create_slider(
                ...
                "slider-bar-protect-right-second_linear", 
                3, 
                time=True
            )

            # Two-segment model
            if modelization_type == "first-model-second_linear":
            ...
            # Three-segment model
            elif modelization_type == "second-model-second_linear":
        ...
        first_part_df, second_part_df = divise_df(df)
        slider_left = create_slider(first_part_df, 0, "slider-bar-protect-left-second_linear", 3, time=True)
        slider_right = create_slider(second_part_df, len(second_part_df)-1, "slider-bar-protect-right-second_linear", 3, time=True)

        return no_update, no_update, no_update, slider_left, slider_right, False, ""
        
    except Exception as e:
        logging.error(f"Error in update_figure_second_linear : {e}/n{traceback.format_exc()}")
        return (
...
@callback(
    ...
    Output("error-toast-app", "is_open", allow_duplicate=True),
    Output("error-toast-app", "children", allow_duplicate=True),
    Input("slider-bar-protect-left-second_linear", 'value'),
    Input("slider-bar-protect-right-second_linear", 'value'),
    State('graph-second_linear', 'figure'),
    State('data_vo2', 'data'),
    State('data_outliers', 'data'),
    prevent_initial_call=True
)
def display_bar_second_linear(slider_value_left, slider_value_right, graph, data_vo2, data_outliers):
    """
    Updates the protection bars' positions in the second linear analysis graph based on the slider values for left and right bars.
    ...
    try:
        if graph:
            ...
            if trigger == "slider-bar-protect-left-second_linear":
            ...
            if trigger == "slider-bar-protect-right-second_linear":
        ...
        logging.error(f"Error in display_bar_second_linear : {e}\n{traceback.format_exc()}")
        return (
            no_update, no_update,
            True, "Une erreur s'est produite. Consultez les journaux pour plus de détails."
        )
...
@callback(
    ...
    Output("error-toast-app", "is_open", allow_duplicate=True),
    Output("error-toast-app", "children", allow_duplicate=True),
    Input('slider-outlier-second_linear', 'value'),
    State('graph-second_linear', 'figure'),
    State('data_vo2', 'data'),
    State("dropdown-model-second_linear", "value"),
    prevent_initial_call=True
)
def outliers_second_linear(slider_value_outlier, graph, data_vo2, model_type):
    """
    Updates the outliers' data and the graph based on the selected slider value for outliers in the second linear analysis.
    ...
    try:
        ...
        logging.error(f"Error in outliers_second_linear : {e}\n{traceback.format_exc()}")
        return (
...
@callback(
    ...
    Output("error-toast-app", "is_open", allow_duplicate=True),
    Output("error-toast-app", "children", allow_duplicate=True),
    Input('prev-button-second_linear', 'n_clicks'),
    Input('next-button-second_linear', 'n_clicks'),
    State('test', 'data'),
    State("data_vo2", "data"),
    State("data_outliers", "data"),
    State('graph-second_linear', 'figure'),
...
def change_analysis_step_second_linear(prev_btn, next_btn, test_id, data_vo2, data_outliers, graph):
    """
    Handles the transition between different steps of the analysis (second linear analysis) and updates the test data.
    ...
    try:
        ...
        if trigger == "prev-button-second_linear" and prev_btn:

            return "/first_linear_analysis", True, False, ""
        
        if trigger == "next-button-second_linear" and next_btn:
        ...
        logging.error(f"Error in change_analysis_step_second_linear : {e}\n{traceback.format_exc()}")
        return (
            no_update, no_update,
            True, "Une erreur s'est produite. Consultez les journaux pour plus de détails."
        )